        ...
        <nav className="w-full max-w-[1800px] mx-auto pointer-events-auto transition-all duration-300 py-3 px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center h-10">
            ...
            <div className="hidden md:flex relative items-center justify-center gap-4 lg:gap-6 xl:gap-8 px-4">
              ...
              {(effectiveRole === 'tenant' || effectiveRole === 'visitor') && (
                <>
                ...
                {showDropdown && (
                  <div className="absolute right-0 mt-3 w-64 bg-white/95 backdrop-blur-md border border-gray-100 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 z-50">
                    <div className="px-5 py-4 border-b border-gray-100 bg-gray-50/50">
                      ...
                      <div className="mt-2">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold ${effectiveRole === 'landlord' ? 'bg-black text-white' : effectiveRole === 'visitor' ? 'bg-amber-50 text-amber-700 border border-amber-100' : 'bg-blue-50 text-blue-700 border border-blue-100'}`}>
                          {effectiveRole === 'landlord' ? 'Landlord' : effectiveRole === 'visitor' ? 'Visitor' : 'Tenant'}
                        </span>
                      </div>
                    </div>
        ...
        {showMobileMenu && (
          <div className="md:hidden mt-3 max-w-7xl mx-auto bg-white/95 backdrop-blur-md border border-gray-200 shadow-2xl rounded-2xl overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300 pointer-events-auto">
            <div className="px-4 py-3 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
              ...
              <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide rounded-full ${effectiveRole === 'landlord' ? 'bg-black text-white' : effectiveRole === 'visitor' ? 'bg-amber-100 text-amber-800' : 'bg-gray-200 text-gray-800'}`}>
                {effectiveRole === 'landlord' ? 'Landlord' : effectiveRole === 'visitor' ? 'Visitor' : 'Tenant'}
              </span>
            </div>
