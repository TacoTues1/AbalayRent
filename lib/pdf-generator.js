...
/**
 * Generates a PDF Statement of Account
 * 
 ...
 */
export async function generateStatementPDF(tenant, payments, period) {
    return new Promise((resolve, reject) => {
        ...
        // --- HEADER ---
        doc.font('Helvetica-Bold').fontSize(20).text('Abalay', { align: 'center' });
        doc.moveDown(0.5);
        ...
        if (payments.length === 0) {
            doc.text('No Payment Record', dateX, y);
            y += 20;
        } else {
        ...
        doc.text('For inquiries, contact support at abalay@gmail.com.', { align: 'center' });
...
/**
 * Generates a PDF Income Statement for Landlords
 * 
 ...
 * @returns {Promise<Buffer>}
 */
export async function generateLandlordStatementPDF(landlord, propertySummary, period, totalIncome, payments = [], propMap = {}) {
    return new Promise((resolve, reject) => {
        // Create new PDF document
        const doc = new PDFDocument({
            margin: 50,
