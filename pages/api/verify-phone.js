...
export default async function handler(req, res) {
  ...
  if (!normalizedPhone || !e164Regex.test(normalizedPhone)) {
    return res.status(400).json({
      error:
        "Invalid phone number format. Please enter a valid phone number (e.g., 09171234567 or +639171234567)",
    });
  }
  ...
  if (action === "send") {
    ...
    if (
      ...
      existing.sentAt > Date.now() - RESEND_COOLDOWN_MS
    ) {
      const waitTime = Math.ceil(
        (existing.sentAt + RESEND_COOLDOWN_MS - Date.now()) / 1000,
      );
    ...
    try {
      ...
      console.error("SMS Gateway failed, trying email fallback:", smsError);

      // Fallback: Send OTP via email using Brevo
      ...
      if (email) {
        try {
          ...
          const emailResponse = await fetch(
            "https://api.brevo.com/v3/smtp/email",
            {
              ...
              body: JSON.stringify({
                ...
                <p>Your verification code for phone number <strong>${normalizedPhone}</strong> is:</p>
                <h1 style="font-size:36px;letter-spacing:8px;text-align:center;color:#111;padding:20px;background:#f3f4f6;border-radius:8px;">${otpCode}</h1>
          ...
          return res.status(200).json({
            success: true,
            message:
              "SMS unavailable. Verification code sent to your email instead.",
            method: "email",
            phone: normalizedPhone,
            waitSeconds: Math.ceil(RESEND_COOLDOWN_MS / 1000),
          });
  ...
  return res
    .status(400)
    .json({ error: 'Invalid action. Use "send" or "verify".' });
}
